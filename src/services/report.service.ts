...
import type { ReportTotals } from "../lib/report-renderer.js";
...
import type {
	ContributorReportingWindow,
	ContributorSummaryPayload,
	ContributorSummaryRecord,
...
export class ReportService {
	...
	private readonly individualActivity: IndividualActivityService;
	private readonly individualSummarizer: IndividualSummarizerService;
	private readonly individualCache: IndividualSummaryCache;
	...
	async generateReport(input: ReportCommandInput): Promise<ReportResult> {
		...
		try {
			...
			if (includeTaskTracker) {
				if (!this.taskTracker || !this.taskTracker.enabled) {
					throw new Error(
					...
					try {
						...
						if (vwData.notes.length === 0) {
							...
							if (accomplishments === null) {
								...
								await vwCache.set(vwHash, accomplishments, { permanent: true });
								await appendUnifiedLog({
									timestamp: new Date().toISOString(),
									runId: "",
									category: "cache",
	...
	private serializePayload(payload: ContributorSummaryPayload): string {
		return JSON.stringify(payload);
	}

	private payloadsMatch(
		a: ContributorSummaryPayload,
		b: ContributorSummaryPayload,
	): boolean {
		return this.serializePayload(a) === this.serializePayload(b);
	}

	private async buildIndividualContributorSummaries(
		members: ReportMemberMetrics[],
		window: ContributorReportingWindow,
	): Promise<ContributorSummaryRecord[]> {
		const payloads = this.individualActivity.buildContributorPayloads({
			members,
			window,
		});
		const cachedEntries = await this.individualCache.readAll();
		const records = new Map<string, ContributorSummaryRecord>();
		const pendingPayloads: ContributorSummaryPayload[] = [];

		for (const payload of payloads) {
			const login = payload.contributor.login;
			const cached = cachedEntries.get(login);
			const fromCache =
				cached &&
				cached.status === "completed" &&
				typeof cached.summary === "string" &&
				this.payloadsMatch(cached.payload, payload);
			...
			await this.individualCache.write({
				login,
				payload,
				status: "pending",
			});
		...
		if (pendingPayloads.length > 0) {
			const summarizerResults =
				await this.individualSummarizer.process(pendingPayloads);
			for (const payload of pendingPayloads) {
				const login = payload.contributor.login;
				const result = summarizerResults.get(login);
				if (!result) {
					const fallbackMessage = "Summarizer did not return a result.";
					const record: ContributorSummaryRecord = {
						login,
						displayName: payload.contributor.displayName,
						status: "failed",
