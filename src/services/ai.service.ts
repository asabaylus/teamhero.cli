...
export class AIService {
	...
	private normalizeText(text: string | null | undefined): string | null {
		...
		const sanitized = text.trim().replace(/\r/g, "");
		return sanitized
			.split("\n")
			.map((line) => line.trim())
			.join("\n");
	...
	private buildRawActivityBlock(member: ReportMemberMetrics): string {
		const lines: string[] = [];
		lines.push(`${member.displayName} (@${member.login})`);
		const prsProcessed =
			(member.prsOpened ?? 0) +
			(member.prsClosed ?? 0) +
			(member.prsMerged ?? 0);
		lines.push(`PRs processed: ${prsProcessed} (merged: ${member.prsMerged})`);
		lines.push(`Commits: ${member.commits}`);
		lines.push(`Reviews: ${member.reviews}`);

		const prHighlights = member.prHighlights.filter(
			(highlight) => !/^No PRs/i.test(highlight),
		);
		if (prHighlights.length > 0) {
			lines.push("Pull Requests:");
			for (const pr of prHighlights) {
				lines.push(`- ${pr}`);
			}
		}

		const commitHighlights = member.commitHighlights.filter(
			(highlight) => highlight.trim().length > 0,
		);
		if (commitHighlights.length > 0) {
			lines.push("Commits:");
			for (const commit of commitHighlights) {
				lines.push(`- ${commit}`);
			}
		}

		const additionalHighlights = member.highlights.filter(
			(highlight) => highlight.trim().length > 0,
		);
		...
		if (
			...
			lines.push("Asana Tasks:");
			for (const task of member.taskTracker.tasks) {
				lines.push(`- ${task.name ?? "Unnamed task"}`);
			}
		} else if (member.taskTracker.message) {
			lines.push(`Asana Note: ${member.taskTracker.message}`);
		}
