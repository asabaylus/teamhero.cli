...
// ---------------------------------------------------------------------------
// Per-section model display/store value tests
// ---------------------------------------------------------------------------

func TestSettingDisplayValue_PerSectionModelEmptyShowsInherit(t *testing.T) {
	for _, key := range []string{
		"AI_TEAM_HIGHLIGHT_MODEL",
		"AI_MEMBER_HIGHLIGHTS_MODEL",
		"AI_INDIVIDUAL_SUMMARIES_MODEL",
		"VISIBLE_WINS_AI_MODEL",
		"AI_DISCREPANCY_ANALYSIS_MODEL",
	} {
		got := settingDisplayValue(key, "")
		if got != "(use primary model)" {
			t.Errorf("settingDisplayValue(%s, '') = %q, want '(use primary model)'", key, got)
		}
	}
}

func TestSettingDisplayValue_PerSectionModelNonEmptyPassthrough(t *testing.T) {
	got := settingDisplayValue("AI_TEAM_HIGHLIGHT_MODEL", "gpt-5")
	if got != "gpt-5" {
		t.Errorf("settingDisplayValue(AI_TEAM_HIGHLIGHT_MODEL, gpt-5) = %q, want gpt-5", got)
	}
}

func TestSettingStoreValue_PerSectionModelInheritToEmpty(t *testing.T) {
	for _, key := range []string{
		"AI_TEAM_HIGHLIGHT_MODEL",
		"AI_MEMBER_HIGHLIGHTS_MODEL",
		"AI_INDIVIDUAL_SUMMARIES_MODEL",
		...
		got := settingStoreValue(key, "(use primary model)")
		if got != "" {
			t.Errorf("settingStoreValue(%s, '(use primary model)') = %q, want empty", key, got)
		}
	}
}

func TestSettingStoreValue_PerSectionModelNonInheritPassthrough(t *testing.T) {
	got := settingStoreValue("VISIBLE_WINS_AI_MODEL", "o4-mini")
	if got != "o4-mini" {
		t.Errorf("settingStoreValue(VISIBLE_WINS_AI_MODEL, o4-mini) = %q, want o4-mini", got)
	}
}

func TestSettingDisplayValue_NonPerSectionKeyUnaffected(t *testing.T) {
	// AI_MODEL is the primary key, not a per-section override — empty should stay empty
	got := settingDisplayValue("AI_MODEL", "")
	if got != "" {
		t.Errorf("settingDisplayValue(AI_MODEL, '') = %q, want empty", got)
	}
}

func TestAiSectionModelOptions(t *testing.T) {
	opts := aiSectionModelOptions()
	if len(opts) != len(aiModelOptions)+1 {
		t.Fatalf("aiSectionModelOptions() has %d items, want %d", len(opts), len(aiModelOptions)+1)
	}
	if opts[0] != "(use primary model)" {
		t.Errorf("aiSectionModelOptions()[0] = %q, want '(use primary model)'", opts[0])
	}
	for i, want := range aiModelOptions {
		if opts[i+1] != want {
			t.Errorf("aiSectionModelOptions()[%d] = %q, want %q", i+1, opts[i+1], want)
		}
	}
}
