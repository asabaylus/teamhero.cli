...
function makeMember(login: string, displayName?: string): Member {
	return {
		id: 1,
		nodeId: "n1",
		login,
...
describe("enrichMemberDisplayNames", () => {
	it("replaces displayName when it equals login and user map has a name", () => {
		const members = [makeMember("lakshmiiyer-lumata")];
		const userMap: UserMap = {
			lakshmi: {
				name: "Lakshmi Iyer",
				github: { login: "lakshmiiyer-lumata" },
			},
		};

		enrichMemberDisplayNames(members, userMap);
		expect(members[0].displayName).toBe("Lakshmi Iyer");
	});

	it("does not overwrite displayName when GitHub already provided a real name", () => {
		const members = [makeMember("johndoe", "John Doe")];
		const userMap: UserMap = {
			john: {
				name: "Jonathan Doe",
				github: { login: "johndoe" },
		...
		enrichMemberDisplayNames(members, userMap);
		expect(members[0].displayName).toBe("John Doe");
	});

	it("handles case-insensitive GitHub login matching", () => {
		const members = [makeMember("DanielBrenner")];
		const userMap: UserMap = {
			daniel: {
				name: "Daniel Brenner",
				github: { login: "danielbrenner" },
			},
		};

		enrichMemberDisplayNames(members, userMap);
		expect(members[0].displayName).toBe("Daniel Brenner");
	});

	it("leaves displayName unchanged when no user map entry exists", () => {
		const members = [makeMember("unknown-user")];
		const userMap: UserMap = {
			alice: {
				name: "Alice",
				github: { login: "alice123" },
		...
		enrichMemberDisplayNames(members, userMap);
		expect(members[0].displayName).toBe("unknown-user");
	});

	it("leaves displayName unchanged when user map entry has no name", () => {
		const members = [makeMember("someuser")];
		const userMap: UserMap = {
			some: {
				github: { login: "someuser" },
			},
		};

		enrichMemberDisplayNames(members, userMap);
		expect(members[0].displayName).toBe("someuser");
	});

	it("returns the members array for convenience", () => {
		const members = [makeMember("test")];
		const result = enrichMemberDisplayNames(members, {});
		expect(result).toBe(members);
	});

	it("enriches multiple members in one call", () => {
		const members = [
			makeMember("danielbrenner-lumata"),
			makeMember("lakshmiiyer-lumata"),
			makeMember("asabaylus", "Asa Baylus"),
		];
		const userMap: UserMap = {
			daniel: {
				name: "Daniel Brenner",
				github: { login: "danielbrenner-lumata" },
			},
			lakshmi: {
				name: "Lakshmi Iyer",
				github: { login: "lakshmiiyer-lumata" },
			...
			asa: {
				name: "Asa B.",
				github: { login: "asabaylus" },
			},
		};

		enrichMemberDisplayNames(members, userMap);
		expect(members[0].displayName).toBe("Daniel Brenner");
		expect(members[1].displayName).toBe("Lakshmi Iyer");
		expect(members[2].displayName).toBe("Asa Baylus"); // GitHub already had the name
	});

	it("handles empty user map gracefully", () => {
		const members = [makeMember("test-user")];
		enrichMemberDisplayNames(members, {});
